...
function SuggestionsModal({ isOpen, onClose, isAuthenticated, userId, isAdmin, apiBasePath = '/api/suggestions', primaryColor = '#0d9488', labels: labelOverrides, loginComponent, authHeader, mode = 'suggestion', entityType, entityId, entityLabel, ModalComponent, }) {
    ...
    if (ModalComponent) {
        return ((0, jsx_runtime_1.jsx)(ModalComponent, { isOpen: isOpen, onClose: onClose, children: modalContent }));
    }
    // Built-in modal (backward compatible)
    return ((0, jsx_runtime_1.jsxs)(jsx_runtime_1.Fragment, { children: [(0, jsx_runtime_1.jsx)("div", { className: "fixed inset-0 bg-black/50 z-[1001]", onClick: onClose }), (0, jsx_runtime_1.jsx)("div", { className: "fixed inset-0 z-[1002] flex items-center justify-center p-4", children: (0, jsx_runtime_1.jsx)("div", { className: "bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-full max-w-lg max-h-[80vh] flex flex-col overflow-hidden", onClick: (e) => e.stopPropagation(), onKeyDown: (e) => {
                        if (e.key === 'Escape')
                            onClose();
